...
function mod_InsertTechnicalData($pid,$jsonEncode,$subcategory,$pname,$update,$pressure){
    $qry = "insert into tbl_technical_details set pro_id = ?,pro_name = ?,pressure= ? ,sub_category = ?, technical_value =?,  status = ?";
    $this->db->query($qry,array($pid,$pname,$pressure,$subcategory,$jsonEncode,1));
} 
...
function excel_InsertTechnicalData($pid,$pname,$pressure,$subcat,$json)
  {
      $query="insert into tbl_technical_details set pro_id=?,pro_name=?,pressure=?,sub_category=?,technical_value=?,status=?";
      $this->db->query($query,array($pid,$pname,$pressure,$subcat,$json,1));
  }
 
function modGetTechnicalMappingpressure($id,$mwp)
{
  $arr    = "";
  $i=0;
        //echo $id.$mwp;die;
    ...
    $data  = $query->row_array();
      if(sizeof($data) > 0){
         ...
         $arr[$i]['sku_no']          = $data['sku_no'];
          ...
          $query1 = $this->db->query("select TD.* from tbl_technical_details TD where  TD.pro_id = ? AND TD.pressure = ? AND TD.status = 1",array($id,$mwp));
          $data1  = $query1->result_array();
      if(sizeof($data1) > 0){
