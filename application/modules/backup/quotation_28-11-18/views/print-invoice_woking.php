 ...
 ?>
  <br/><h2><?=$technicaldetails[0]['pro_name'];?></h2><br />
   <?php 
      ...
      if(sizeof($technical_value) != "") {
       ...
       foreach ($technical_value as  $techDt) {
        ...
        ?>
         ...
         <tr ><!-- style="<?=($p%600)==0?'display: block;page-break-before: always;':'';?>" -->
           <td width="40%"><?=$techDt['name'].'-----'.($p%600).'----'.$p;?></td>
           <td width="60%"><?=$techDt['value'].'&nbsp;&nbsp;'.$techDt['entity'];?>
    ...
    ?>
       <!-- <input type="hidden"  value="<?=$totalRow;?>" id="rowheight"> -->
      </tbody>
  </table>
<input type="hidden" name="techname[]" value="<?=$techname;?>" idval="<?=$dt['id'];?>">
<?php 
} }  ?>
<!-- <input type="text"  value="<?=$totalRow;?>" id="rowheight">
<input type="hidden" value="<?=$m;?>" id="scrollheight">
<input type="text" value="<?=json_encode($arr2);?>" id="scrollqqq"> -->
</table>
