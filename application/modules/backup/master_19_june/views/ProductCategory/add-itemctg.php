                        ...
                        <div class="table-responsive">
                          <table class="table table-striped table-bordered table-hover dataTables-example1" >
							...
							<tr class="gradeC record" data-row-id="<?=$rows['id'];?>">
								...
								<th><?=$rows['type']==1?'Product':'Quotation';?></th>
                         ...
                         <?php if($view!=''){ ?>
                             <button class="btn btn-default modalEditItem" type="button" data-toggle="modal" data-target="#modal-1" data-backdrop='static'  typeid = "<?=$rows['type'];?>" arrt = "<?=$rows['name'];?>" cat_id ="<?=$rows['parent_id'];?>" onclick ="editRow(this.id);"  id="<?=$rows['id'];?>" data-keyboard='false'> <i class="fa fa-eye"></i> </button>
                         <?php } if($edit==''){ ?>  
                         <a  id="<?=$rows['id'];?>" typeid = "<?=$rows['type'];?>"  arrt = "<?=$rows['name'];?>" cat_id ="<?=$rows['parent_id'];?>" onclick ="editRow(this.id);" class="btn btn-default modalEditItem"  data-toggle="modal" data-target="#modal-1" >&nbsp; <i class="icon-pencil"></i> &nbsp; </a> 
                          <?php } ?>      
