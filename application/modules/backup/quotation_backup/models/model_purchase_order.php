...
//echo "select H.subject,H.invoice_date,H.purchase_no,H.contant,C.first_name,C.email,C.address1,C.city,S.stateName from  tbl_quotation_order_hdr H,tbl_contact_m C,tbl_state_m S where C.contact_id = H.contactid AND S.stateid = C.state_id AND H.quotationid =$id";
    ...
    $query=$this->db->query("select H.subject,H.pricetext,H.invoice_date,H.purchase_no,H.contant,C.title_deal,C.contact_person,C.org_name,C.mobile,C.address1,C.city,C.state_id,C.person_email,C.person_name from  tbl_quotation_order_hdr H,tbl_contact_m C where C.contact_id = H.contactid  AND H.quotationid =?",array($id));
...
function getinvoicetechdetails($id){
      //echo "select D.*,I.grade as dgrade from  tbl_technical_details D, tbl_quotation_order_dtl I  where I.productid = D.pro_id AND I.quotationid = $id AND D.status = 1";
        ...
        $query=$this->db->query("select D.*,I.grade as dgrade from  tbl_technical_details D, tbl_quotation_order_dtl I  where I.productid = D.pro_id AND I.quotationid = ? AND D.status = ?",array($id,1));
