...

{
    ...
    $url   = site_url('/quotation/quotation/manage_quotation?');
	...
	if($_GET['entries']!="")
      ...
      $totalData   = $this->model_purchase_order->count_allquotation($table_name,'A',$this->input->get());
    ...
    if($_GET['entries']!="" && $_GET['filter'] != 'filter')
	  ...
	  $url        = site_url('/quotation/quotation/manage_quotation?entries='.$_GET['entries']);

    else

       $url        = site_url('/quotation/quotation/manage_quotation?entries='.$_GET['entries'].'&purchase_no='.$_GET['purchase_no'].'&invoice_date='.$_GET['invoice_date'].'&contactid='.$_GET['contactid'].'&grand_total='.$_GET['grand_total'].'&filter='.$_GET['filter']);
    ...
    // print_r($data['mapcategory']);
     ...
     if($this->input->get('filter') == 'filter')   ////filter start ////
        ...
        $data['result']       = $this->model_purchase_order->filterListquotation($pagination['per_page'],$pagination['page'],$this->input->get());

     else	

	    $data['result'] = $this->model_purchase_order->invoice_data($pagination['per_page'],$pagination['page']);
...
public function insertQuotation(){
		...
		$table_name_acc = 'tbl_accessories_quotation';
...

		{
			...
			//echo $quotationMapedValue[$i];die;
			if($qty[$i]!=''){
		...
		//------------------------------Accessories Quotation Mapping---------------------

	    if($access_row!=0)
	    		{
				for($i=0;$i<$access_row;$i++)
						{
							
							$quotation_access=array(
							
								'Quotation_id' => $lastHdrId,
							
								'acc_category' => $acc_catid[$i],
						
								'acc_subcategory' => $acc_subid[$i],
					
								'acc_price' =>  $mprice[$i],

								'acc_description' => $mdescrip[$i],
							...
							$this->Model_admin_login->insert_user($table_name_acc,$quotation_access);
						}

				}
				//echo $lastHdrId."qid".$access_row."rows";die;	    
...
public function print_quotation(){
	...
	if($this->session->userdata('is_logged_in')){
     ...
     $data['technicaldetails'] = $this->model_purchase_order->getinvoicetechdetails($this->input->get('id'),$this->input->get('pressure'));
     ...
     //echo print_r($scopedetails);die;

     $data['accessories'] = $this->model_purchase_order->getquotationaccessories($this->input->get('id'));

     //print_r($data['accessories']);die;
...
public function print_quotation_det(){
	...
	if($this->session->userdata('is_logged_in')){
     ...
     $data['technicaldetails'] = $this->model_purchase_order->getinvoicetechdet($this->input->get('id'),$this->input->get('pressure'));
     ...
     $this->load->view('quotation/print-invoice-det',$data);
...
public function ajex_selectQuotation(){

	$data['id'] = $this->input->post('id');

    $this->load->view('viewQuotationSelectBox',$data);     
 ...
 public function ajex_nextIncrementId(){
 	...
 	$datevall    = $this->input->post('dateval');

 	$explode_val = explode("-",$datevall);

    $year        = substr($explode_val[0], -2);



    $query    = $this->db->query("SELECT auto_increment FROM INFORMATION_SCHEMA.TABLES WHERE table_name = 'tbl_quotation_order_hdr'");
	...
	if(sizeof($result) > 0){
	 ...
	 echo 'TD/'.$year.$explode_val[1].$explode_val[2].'/'.$result['auto_increment'];
 ...
 public function ajex_selectgrade(){

 	$pid  = $this->input->post('id');

 	$data = $this->model_purchase_order->selectgrade($pid);
 	...
 	if(sizeof($data) > 0){
       ...
       foreach ($data as $dt) { 
         ...
         echo '<option value="'.$implode.'">'.$dt.'</option>';
	...
	public function quotationdetails()

		{
			...
			$data=array();	
			//$result['pri_col']='Accessory_id';
			//$result['id']=$_POST['acc_id'];
			$data['result']=$this->model_purchase_order->getaccessories($_POST['acc_id'],$pri_col);
			//echo $result->acc_price;
			$this->load->view('accessories_table',$data);
			//echo $result->acc_price;
		}
	public function get_accessories_subcat()
		{

			$accid=$_POST['acc_id'];
			$data['accid']=$accid;
			$this->load->view('get-accessories-subcat',$data);
			
			//echo $_POST['acc_id']."acc_cat_id";
		}
}