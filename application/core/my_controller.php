...
// function wordconvertor(){
// echo  $data['WordConvertor']    = $this->WordConvertor->loadfunctionnamess();
// }
...
function delete_purchase_order_data() {
	...
	$this->load->model('Model_admin_login');
		...
		$table_name =$dataex[1];
		$pri_col =$dataex[2];
		$table_name1 =tbl_quotation_order_dtl;
		$pri_col1 =quotationid;
...
public function duplicate_po_check()
{

	$po_id=$_GET['po_id'];
	$this->db->where('purchaseid', $po_id);
    $query = $this->db->get('tbl_purchase_order_hdr');
    $count_row = $query->num_rows();
    if ($count_row > 0) {
		echo "False";
        return FALSE; 
