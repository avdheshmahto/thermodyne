...
function editItem(ths) {
  ...
  var rowValue = $(ths).attr('arrt');
  var button_property = $(ths).attr('property');
  $("#title-area-name").text(button_property);
...
$("#excelTechnicalMapping").validate({
    rules: {
      file_name:"required",
      pressure:"required",
    },
      submitHandler: function(form) {
        ur = "<?=base_url('master/Item/insertExcelTechnicalData');?>";
        var formData = new FormData(form);
        //console.log(formData);
            $.ajax({
                type : "POST",
...
function ajex_ItemListData(){
  ur = "<?=base_url('master/Item/ajex_ItemListData');?>";
    $.ajax({
      ...
      success: function(data){
        //$("#listingData").hide();
        $("#listingData").empty().append(JSON.stringify(data));
...
$(document).ready(function(){
  ...
  if(zNodes != ""){
    ...
    $(document).delegate("#technical_main","click",function(){

       var className = $(this).parent().parent().attr('class');
       var splitval = className.split("_");
       var makeClass = splitval[0]+'_row_'+splitval[1];
       //alert(makeClass);
    ...
    if(subcatg == ""){
     ...
     $('#resultarea3').empty().text('Please Enter  SubCategory !');
      ...
      var incrementval = isNaN(value) ? 0 : value;
          incrementval++;
      var subcatg_replace =  subcatg.replace("(", "-");
        subcatg_replace =  subcatg_replace.replace(")", "-");

       $('#incrementval').val(incrementval); ///name="technical['+subcatg+incrementval+']"
       $('#technicalTable').append('<tr style="color: #fbfbfb;background: #929090;" class="'+subcatg_replace+'_'+incrementval+'"><td>Subcategory</td><td ><input  type ="text" class="form-control"  value="'+subcatg+'"></td><td></td><td></td><td><i class="fa fa-trash  fa-2x" id="technical_main" aria-hidden="true"></i></td></tr>');
      // $('#sub_catg').val("");
...
function addtechnical(){
    ...
    if(tname == ""){
		//alert();
     $('#resultarea4').empty().text('Please Enter Item !');
      ...
      var rowValue = isNaN(rowVal) ? 0 : rowVal;
       ...
       $('#rowVal').val(rowValue);
       $('#technicalTable').append('<tr class="'+subcatg_replace+'_row_'+value+'"><td><input  type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][name]" value="'+tname+'"></td><td><input  type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][entity]" value="'+tentity+'"></td><td><input  type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][value]" value="'+tvalue+'"></td><td><input type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][grade]" value="'+tgrade+'"></td><td><i class="fa fa-trash  fa-2x" id="quotationdel" aria-hidden="true"></i></td></tr>');

     /*  alert('<tr class="'+subcatg_replace+'_row_'+value+'"><td><input  type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][name]" value="'+tname+'"></td><td><input  type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][entity]" value="'+tentity+'"></td><td><input  type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][value]" value="'+tvalue+'"></td><td><input type ="text" class="form-control" name="technical['+subcatg_replace+'_'+value+']['+rowValue+'][grade]" value="'+tgrade+'"></td><td><i class="fa fa-trash  fa-2x" id="quotationdel" aria-hidden="true"></i></td></tr>');*/
...
function exceltechnicaldata(pid){
  //alert(pid);
 
 var ur ="<?=base_url('master/Item/ajax_inserttechnicaldata');?>";
    ...
    $.ajax({
      ...
      success : function (data) {
        // var jsonpars = JSON.parse(data);
         ...
         $("#excelTechnicalMapping").empty().append(data).fadeIn();
        $('input[name=pid]').val(pid);
         //console.log(JSON.parse(jsonpars['price']));
      }
    });
...
$("#AccessoriesForm").validate({
    rules: {
      ...
      acc_subcategory: "required",
      acc_price: "required"
    },
      submitHandler: function(form) {
        ur = "<?=base_url('master/Accessories/insert_accessories');?>";
        var formData = new FormData(form);
            $.ajax({
                type : "POST",
                url  :  ur, 
...
function AccessoriesList()
  {
    $.ajax({
        url:"<?php echo base_url()?>master/Accessories/AccessoriesList",
        type:"POST",
        success:function(html)
        {
         
          $("#listingData").empty().append(html).fadeIn();
        }

    });
  }

function editAccessory(ths) {
  //console.log('edit ready !');
  $('.error').css('display','none');
  var rowValue = $(ths).attr('arrt');
  var button_property = $(ths).attr('property');
  ...
  //console.log(rowValue);
   if(rowValue !== undefined)
    var editVal = JSON.parse(rowValue);
    if(button_property != 'view')
      $('#Accessory_id').val(editVal.Accessory_id);

        $('#acc_category').val(editVal.acc_category).prop('selected', true);
     // $('#scope').val(editVal.scope_doc).prop('selected', true);
    ...
    //  var valArr  = editVal.color; 
     // var dataarray=valArr.split(",");
     // $('#color').val(dataarray);
      $('#acc_subcategory').val(editVal.acc_subcategory);
      $('#acc_price').val(editVal.acc_price);
      $('#acc_description').val(editVal.acc_description);
     
      if(button_property == 'view'){
     $('.top_title').html('View');
       $('#button').css('display','none');
       $("#AccessoriesForm :input").prop("disabled", true);
       
      }else{
    $('.top_title').html('Update');
       $('#button').css('display','block');
       $("#AccessoriesForm :input").prop("disabled", false);
       
      }

};

function gettechdetails(ths)
  {
    var a=$(ths).val();
    var pid=$('input[name=pid]').val();
    //alert(id);
    var ur ="<?=base_url('master/Item/ajax_getTechnicalMapping_pressure');?>";
    var ids="id="+pid+"&mwp="+a;
    //alert(ids);
    //$("#technicalTable").empty().append("hello");
    $.ajax({
      type : "POST",
      url  :  ur, 
      data : ids, 
